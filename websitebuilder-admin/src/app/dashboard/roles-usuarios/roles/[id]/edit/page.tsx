  ...
  const fetchRole = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error fetching role:', response.status);
        alert(t('rolesUsers.roleNotFound', 'Role not found'));
        router.push('/dashboard/roles-usuarios?tab=roles');
      }
    } catch (error) {
      console.error('Error fetching role:', error);
      alert(t('rolesUsers.loadError', 'Error loading role'));
      router.push('/dashboard/roles-usuarios?tab=roles');
    } finally {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {t('rolesUsers.updateRoleDetails', 'Update role details and permissions')}
                  </p>
                </div>
              </div>
            </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span className="hidden sm:inline">{t('common.update', 'Update Role')}</span>
                  </>
                )}
              </button>
            </div>
