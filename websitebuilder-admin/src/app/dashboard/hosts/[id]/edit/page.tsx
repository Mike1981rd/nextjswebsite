...
export default function EditHostPage() {
  ...
  const [formData, setFormData] = useState<UpdateHostDto>({
    firstName: '',
    lastName: '',
    email: '',
    phoneNumber: '',
    bio: '',
    dateOfBirth: '',
    yearStartedHosting: new Date().getFullYear(),
    aboutMe: '',
    location: '',
    work: '',
    attributes: [],
  ...
  useEffect(() => {
    const loadHost = async () => {
      try {
        ...
        const formattedDate = host.dateOfBirth ? 
        ...
        setFormData({
          ...
          yearStartedHosting: host.yearStartedHosting || new Date().getFullYear(),
          ...
          isPhoneVerified: host.isPhoneVerified || false,
          isEmailVerified: host.isEmailVerified || false,
          isIdentityVerified: host.isIdentityVerified || false,
          responseTimeMinutes: host.responseTimeMinutes || 60,
          profilePicture: host.profilePicture || '',
          isSuperhost: host.isSuperhost || false,
          isActive: host.isActive !== undefined ? host.isActive : true
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.put(`/hosts/${hostId}`, submitData);
      router.push('/dashboard/hosts');
    } catch (error: any) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm">
        ...
        <div className="hidden sm:block p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {t('hosts.editHostDescription', 'Update host information')}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {t('hosts.fields.aboutMe', 'About Me')}
              </label>
              <div className="relative">
                <FileText className="absolute left-3 top-3 text-gray-400 w-4 h-4" />
                <textarea
                  ...
                  onChange={handleChange}
                  rows={5}
                  placeholder={t('hosts.placeholders.aboutMe', 'Tell guests more about yourself, your interests, and what makes you a great host...')}
                  className="w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2"
                  style={{ '--tw-ring-color': primaryColor } as any}
                />
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {t('hosts.fields.yearStartedHosting', 'Hosting Since Year')}
              </label>
              <div className="relative">
                <Calendar className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                <input
                  ...
                  value={formData.yearStartedHosting}
                  onChange={handleChange}
                  min="1900"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {t('hosts.fields.location', 'Location')}
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t('hosts.placeholders.location', 'e.g., Madrid, Spain')}
                  className="w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2"
                  style={{ '--tw-ring-color': primaryColor } as any}
                />
              </div>
            ...
            <div className="lg:col-span-2">
              ...
              <input
                ...
                value={formData.work}
                onChange={handleChange}
                placeholder={t('hosts.placeholders.work', 'e.g., Graphic Designer, Teacher, etc.')}
            ...
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {t('hosts.fields.attributes', 'Attributes/Qualities')}
              </label>
                    ...
                    onKeyPress={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        if (attributeInput.trim()) {
                          setFormData({
                            ...formData,
                            attributes: [...(formData.attributes || []), attributeInput.trim()]
                          });
                          setAttributeInput('');
                        }
                      }
                    }}
                    placeholder={t('hosts.placeholders.addAttribute', 'e.g., Friendly, Punctual, Communicative')}
                    className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2"
                    style={{ '--tw-ring-color': primaryColor } as any}
                  />
                  <button
                    type="button"
            ...
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {t('hosts.fields.hobbies', 'Hobbies & Interests')}
              </label>
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder={t('hosts.placeholders.addHobby', 'e.g., Photography, Hiking, Cooking')}
                    className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2"
                    style={{ '--tw-ring-color': primaryColor } as any}
                  />
                  <button
