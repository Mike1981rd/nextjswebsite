...
export function EditorPreview({ deviceView = 'desktop' }: EditorPreviewProps) {
  ...
  const [isAnnouncementAnimating, setIsAnnouncementAnimating] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    if (announcementConfig?.animationStyle === 'infinite-marquee') {
      setCurrentAnnouncementIndex(0); // Reset to first for marquee
      return;
    }
    
    // Only set interval for autoplay with multiple announcements (not marquee)
    if (announcementConfig?.autoplay?.mode === 'one-at-a-time' && 
        announcements.length > 1 && 
        announcementConfig?.animationStyle !== 'infinite-marquee') {
      ...
      }, (announcementConfig.autoplay.speed || 5) * 1000);

      return () => clearInterval(interval);
    }
    
    // Reset index when no autoplay
    if (announcementConfig?.autoplay?.mode === 'none') {
      setCurrentAnnouncementIndex(0);
    }
  }, [
    structuralComponents?.announcementBar?.autoplay?.mode,
    structuralComponents?.announcementBar?.autoplay?.speed,
    structuralComponents?.announcementBar?.animationStyle,
    structuralComponents?.announcementBar?.announcements?.length
  ]);
  ...
  const renderSectionPreview = (section: Section) => {
    ...
    switch (section.type) {
      case SectionType.ANNOUNCEMENT_BAR:
        ...
        const announcementScheme = colorSchemes?.schemes?.[parseInt(announcementConfig?.colorScheme || '1') - 1];
        const borderRadius = `${announcementConfig?.edgeRounding || 0}px`;
        
        // Debug logging
        console.log('=== ANNOUNCEMENT BAR CONFIG ===');
        console.log('Full config:', announcementConfig);
        console.log('Social media enabled:', announcementConfig?.socialMediaIcons?.enabled);
        console.log('Social media showOnDesktop:', announcementConfig?.socialMediaIcons?.showOnDesktop);
        console.log('Social URLs:', announcementConfig?.socialMediaUrls);
        console.log('Language selector:', announcementConfig?.languageSelector);
        console.log('Currency selector:', announcementConfig?.currencySelector);
        console.log('Show navigation arrows:', announcementConfig?.showNavigationArrows);
        console.log('Announcements count:', announcementConfig?.announcements?.length);
        
        // Get active announcement with state for navigation
        const announcements = announcementConfig?.announcements || [];
        const activeAnnouncement = announcements[currentAnnouncementIndex || 0];
        
        console.log('Announcement Animation Style:', announcementConfig?.animationStyle);
        console.log('Animation Class Applied:', getAnnouncementAnimationClass(announcementConfig?.animationStyle));
        console.log('Current Index:', currentAnnouncementIndex);
        console.log('Animation Key:', animationKey);
        console.log('Show only on home page:', announcementConfig?.showOnlyHomePage);
        console.log('Selected page type:', selectedPageType);
        
        // Determine max width based on width setting
        const getMaxWidth = () => {
          switch(announcementConfig?.width) {
        ...
        if (announcementConfig?.showOnlyHomePage && selectedPageType !== PageType.HOME) {
          return null;
        }
        
        // Check if enabled and has announcements
        if (!announcementConfig?.enabled || announcements.length === 0) {
          return null;
        }
        ...
        return (
          <div 
            style={{ 
              backgroundColor: announcementScheme?.background || '#000000',
              color: announcementScheme?.text || '#ffffff',
              borderRadius: announcementConfig?.width !== 'screen' ? borderRadius : '0',
              maxWidth: getMaxWidth(),
              margin: announcementConfig?.width !== 'screen' ? '0 auto' : '0'
            }}
            className="relative"
          >
            <div className="flex items-center justify-between py-2 px-4">
              ...
              <div className="flex items-center gap-3">
                {announcementConfig?.socialMediaIcons?.enabled && announcementConfig?.socialMediaIcons?.showOnDesktop && (
                  <div className="flex items-center gap-3">
                    {Object.entries(announcementConfig.socialMediaUrls || {})
                      .filter(([_, url]) => url)
                      .slice(0, 5)
                      .map(([platform, url]) => (
                        <a key={platform} href={url as string} className="hover:opacity-70" aria-label={platform}>
                          {renderSocialIcon(platform, announcementConfig?.socialMediaIcons?.iconStyle || 'solid')}
                        </a>
                      ))}
                  </div>
                )}
              ...
              <div className="flex-1 flex items-center justify-center gap-36">
                {/* Left Arrow */}
                {announcementConfig?.showNavigationArrows && announcements.length > 1 && (
                  <button 
                ...
                {announcementConfig?.animationStyle === 'infinite-marquee' && announcements.length > 0 ? (
                  // Marquee animation - special structure
                  <div className="text-sm overflow-hidden whitespace-nowrap" style={{ minHeight: '24px', width: '100%' }}>
                    <div 
                      className="inline-flex"
                      style={{
                        animation: 'marquee 8s linear infinite'  // Más rápido aún
                      }}
                    >
                  ...
                  <div className="text-center text-sm flex items-center gap-2 relative overflow-hidden" style={{ minHeight: '24px' }}>
                    <div 
                      className={`flex items-center gap-2 ${getAnnouncementAnimationClass(announcementConfig?.animationStyle)}`}
                      key={`announcement-${currentAnnouncementIndex}-${animationKey}`}
                    >
                      {/* Render actual icon if exists */}
                      {activeAnnouncement?.icon && renderAnnouncementIcon(activeAnnouncement.icon)}
                      {activeAnnouncement?.link ? (
                        <a href={activeAnnouncement.link} className="hover:underline">
                          {activeAnnouncement.text || 'Welcome to our store!'}
                        </a>
                      ) : (
                        <span>{activeAnnouncement?.text || 'Welcome to our store!'}</span>
                      )}
                    </div>
                  </div>
                )}
              ...
              {/* Right side - Language/Currency selectors */}
              <div className="flex items-center gap-8">
                {announcementConfig?.languageSelector?.enabled && announcementConfig?.languageSelector?.showOnDesktop && (
                ...
                {announcementConfig?.currencySelector?.enabled && announcementConfig?.currencySelector?.showOnDesktop && (
                  <div className="relative">
                    <select 
                      className="bg-transparent text-xs border-0 outline-none cursor-pointer appearance-none pr-4"
                      style={{ color: 'inherit' }}
